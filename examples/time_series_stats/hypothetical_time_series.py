...
class ExponentialMovingAverageOfInterval:
    ...
    def plot_time_series(self):
        ...
        plt.plot(self.input_time_series_interval.time_points, self.input_time_series_interval.tokens, label="Input")
        plt.plot(self.ema_time_series_interval.time_points, self.ema_time_series_interval.tokens, label="EMA")
        plt.xlabel("Time")
        plt.ylabel("Value")
...
class SimulateData:
    ...
    def generate_sine_wave_with_noise(
        *,
        ...
        sine_wave_tokens = SimulateData.sine_wave(timestamps, period_ms=period_ms) * amplitude + shift
        noisy_tokens = SimulateData.add_noise(sine_wave_tokens, noise_std * amplitude, rng)
        return timestamps, noisy_tokens
    ...
    def generate_sine_wave_with_noise_from_parameters(parameters: TimeSeriesGeneratingParameters) -> TimeSeriesInterval:
        ...
        timestamps, tokens = SimulateData.generate_sine_wave_with_noise(
            start=parameters.start,
            end=parameters.end,
            n=parameters.n,
            amplitude=parameters.amplitude,
        ...
        return TimeSeriesInterval(parameters.start, parameters.end, datetime_timestamps, tokens)
...
class SeriesStatistics:
    ...
    def exponentially_decaying_mean_weighted_tokens_in_time_window(
        time_points: Sequence[datetime],
        tokens: np.ndarray,
        start_of_window: datetime,
        end_of_window: datetime,
        ...
        """Compute weighted tokens for each time point in the time window."""
        if len(time_points) != len(tokens):
            raise ValueError("time_points and tokens must have the same length")
        ...
        tokens_at_time_points_in_window = tokens[np.isin(time_points, time_points_in_window)]
        weighted_average = np.average(tokens_at_time_points_in_window, weights=weights)
    ...
    def exponential_moving_average(
        ...
        for start, end in window_starts_and_ends:
            _, mean_weighted_value = SeriesStatistics.exponentially_decaying_mean_weighted_tokens_in_time_window(
            ...
            mean_weighted_tokens.append(mean_weighted_value)
            window_ends.append(end)
        return window_ends, mean_weighted_tokens
    ...
    def datetime_interval_ema(
        ...
        window_ends, ema_tokens = SeriesStatistics.exponential_moving_average(
            interval.time_points, interval.tokens, start_time, end_time, decay_parameter
        )
        return ExponentialMovingAverageOfInterval(
            input_time_series_interval=deepcopy(interval),
            ema_time_series_interval=TimeSeriesInterval(start_time, end_time, window_ends, np.array(ema_tokens)),
            decay_parameter=decay_parameter,
        )
