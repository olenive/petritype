...
def _(mo):
    mo.md(
        r"""
        # Match up tokens — move unmatched

        Based on `01_match_lengths`, with one change: when no more tokens can be matched,
        the remaining unmatched tokens are moved to new places to indicate that they could
        not be matched up (so the run terminates gracefully instead of raising).
...
def _(Optional, Union):
    # NOTE: In practice you would probably want to define pydantic models for the I/O types.
        ...
        for s in strings:
            for length in lengths:
                if len(s) == length:
                    ...
                    return strings, lengths, (s, length), None, None
        # No match found: move the remainder to the "unmatched" places.
